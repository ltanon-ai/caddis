//! json.rs — minimal JSON value parser + serializer (READ-ONLY PORT).
//! Origin: crates/caddis-memory/src/json.rs (caddis-remember slice (a)),
//! vendored verbatim — the memory crate is a domain crate and the
//! deliberation organ must not layer on it; caddis-core's TCB law keeps
//! serde_json dev-only until the schema freezes. Copies now live in
//! caddis-memory, caddis-voice AND HERE — any parser FIX lands in ALL
//! THREE until the parser graduates to caddis-core. Full grammar:
//! null/true/false, numbers, strings with all escapes incl. surrogate
//! pairs, arrays, objects; depth-capped at 128 (fail-closed on
//! pathological input); trailing non-whitespace after a document is an
//! error. P1 slice 1 (BUILD-QUEUE r2-organs-rewrite) needs it twice:
//! reading the seed `models.json` (nested provider catalog) and the
//! card-stream/view encode/parse round-trip.
