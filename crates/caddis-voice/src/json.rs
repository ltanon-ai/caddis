...
//! vendored verbatim — caddis-memory is a domain crate and the voice organ
//! must not layer on it; caddis-core's TCB law keeps serde_json dev-only
//! until the schema freezes. Any parser FIX lands in BOTH copies until the
//! parser graduates to caddis-core. Full grammar: null/true/false, numbers,
//! strings with all escapes incl. surrogate pairs, arrays, objects;
//! depth-capped at 128 (fail-closed on pathological input); trailing
//! non-whitespace after a document is an error.
