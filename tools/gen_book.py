...
PRIORITY = [
    ...
    "docs/API.md",
    "docs/TELEMETRY.md",
    "docs/54-rework-verdict.md",
    "docs/41-riverbed-context.md",
    "docs/42-pools-dna.md",
    "docs/43-clear-water.md",
    "docs/43-clear-water-vision.md",
    "docs/55-forge-gate-design.md",
    "docs/26-testing-doctrine.md",
    "docs/19-gap-sweep.md",
    "docs/11-three-radicals.md",
    "docs/12-llm-callsites.md",
    "AGENTS.md",
...
def git(args: list[str]) -> str:
    # `args` is a LIST of arguments, not a string — it is concatenated onto the
    # base command. The annotation said `str` and every caller passed a list,
    # which Sonar flagged twice as CRITICAL: on `str` the `+` would silently
    # build a character-wise argv rather than failing.
...
# Longest prefix first so a deeper heading is never matched by a shallower
# rule. `"## x".startswith("# ")` is already False, but ordering by length
# keeps that a property of the table rather than of the string literals.
_HEADINGS = (("### ", "h4"), ("## ", "h3"), ("# ", "h2"))


def _render_fence(lang, buf):
    """One closed code fence as HTML. Mermaid passes through for client render."""
    content = esc("\n".join(buf))
    if lang == "mermaid":
        return f'<div class="mmwrap"><pre class="mermaid">{content}</pre></div>'
    return f'<pre class="code"><code>{content}</code></pre>'


def _render_line(line):
    """One markdown-lite line OUTSIDE a code fence, as HTML."""
    for prefix, tag in _HEADINGS:
        if line.startswith(prefix):
            return f"<{tag}>{esc(line[len(prefix) :])}</{tag}>"
    if re.match(r"^\s*[-*] ", line):
        return '<div class="li">' + esc(line.lstrip()[2:]) + "</div>"
    if line.startswith("|"):
        return f'<div class="trow">{esc(line)}</div>'
    if line.strip():
...
def chapter(idx, rel, fp):
    ...
    mtime = datetime.datetime.fromtimestamp(os.path.getmtime(fp)).strftime(
        "%Y-%m-%d %H:%M"
    )
    bh = blob_hash(rel)
    ...
    parts.append(
        f'<div class="chmeta"><span class="path">{esc(rel)}</span>'
        f'<span class="stamp">mtime {mtime} · blob {bh}</span></div>'
    )
    # The loop owns ONLY the fence state machine; rendering lives in the two
    # helpers above. Sonar measured this function at cognitive complexity 21
    # against a limit of 15 — and the operator cap is 10.
    in_code, lang, buf = False, "", []
    for line in body.splitlines():
        if not in_code and line.startswith("```"):
            in_code, lang, buf = True, line[3:].strip(), []
        elif in_code and line.strip() == "```":
            in_code = False
            parts.append(_render_fence(lang, buf))
        elif in_code:
            buf.append(line)
        else:
...
def main():
    ...
    print(
        f"BOOK.html: {len(files)} chapters, {len(html_doc) // 1024} KB, HEAD {head} @ {gen_ts}"
    )
...
<script src="https://cdn.jsdelivr.net/npm/mermaid@10.9.8/dist/mermaid.min.js" integrity="sha384-N3QqR/7q+xm3BGX+CBbNI8AUmRRqcsDzToy+0z1NLDI0QmTKW8zvwLvqulJgk3dP" crossorigin="anonymous" referrerpolicy="no-referrer"></script>
<style>
:root{
...
#side h1{font:700 20px/1.2 Georgia,serif;color:var(--gold);padding:8px 20px 2px}
...
h2{color:var(--gold);font:700 24px/1.25 Georgia,serif;margin:18px 0 8px;border-bottom:1px solid var(--line);padding-bottom:6px}
h3{color:var(--gold-b);font:700 18px Georgia,serif;margin:16px 0 6px}
h4{color:var(--teal);font:700 15px Georgia,serif;margin:12px 0 4px}
p{margin:7px 0}
...
.mmwrap:before{content:"mermaid";display:block;font:10px monospace;margin:-14px 0 8px -14px;padding:2px 8px;background:var(--gold);color:#101014;border-radius:6px 0 6px 0;position:relative;top:0}
...
<script>if(globalThis.mermaid){mermaid.initialize({startOnLoad:true,theme:'dark',themeVariables:{primaryColor:'#2a2a33',primaryTextColor:'#e8e6e3',primaryBorderColor:'#d7ba7a',lineColor:'#7fbfb0',fontSize:'13px'}});}else{document.querySelectorAll('.mermaid').forEach(e=>e.style.color='#6e6e78');}</script>
</body></html>"""
